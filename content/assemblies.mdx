...
the table `assembly` from the [SQLite metadata database](/metadata_sqlite).
...
in the file [file_list.all.latest.tsv.gz](https://osf.io/zxfmy/files/3xs6h).
This file should have all the information you need.
Older files and files split by dataset are also available - see
the folder "File Lists" in the top level of the [Assembly component on OSF](https://osf.io/zxfmy/).
...
* `sylph_species` = inferred species call from running sylph on the reads.
  ...
  before 202505. See the [species calls](/species_id) page for more
  details
...
then this is the info in [file_list.all.latest.tsv.gz](https://osf.io/zxfmy/files/3xs6h):
...
See the [Sylph section](/species_id) of the species page for details.
...
but were removed while [migrating to OSF](/ebi2osf).)
